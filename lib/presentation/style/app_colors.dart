import 'package:flutter/cupertino.dart';

abstract class AppColors {
  static const Color bgLight = Color(0xFFF5F5F5);
  static const Color greenLight = Color(0xFF5DFF00);
  static const Color textLight = Color(0xFF3D3D3D);
  static const Color outputBgdLight = Color(0xFF121212);

  static const Color bgDark = CupertinoColors.darkBackgroundGray;
  static const Color greenDark = Color(0xFF669900);
  static const Color textDark = Color(0xFFE8E8E8);
  static const Color outputBgdDark = Color(0xFF121212);

  static const Color grayBG = CupertinoColors.darkBackgroundGray;
  static const Color white = CupertinoColors.white;
  static const Color green = CupertinoColors.activeGreen;
  static const Color orange = CupertinoColors.activeOrange;
  static const Color inactiveText = Color(0xff5e5e5e);
  static const Color red = CupertinoColors.destructiveRed;
}
